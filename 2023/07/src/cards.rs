...
impl HandStrength {
    fn from_cards_with_joker(cards: &[Face; 5]) -> Self {
        ...
        match jokers {
            0 => base,
            1 => {
                // only 1 joker, so we account for that in the transformations
                match base {
                    ...
                    _ => {
                        panic!("impossible value {base:?} for hand with 1 joker")
                    }
                }
            }
            2 => {
                // 1 pair is caused by jokers
                match base {
                    HandStrength::P => HandStrength::T,
                    HandStrength::TP => HandStrength::F4,
                    HandStrength::FH => HandStrength::F5,
                    _ => {
                        panic!("impossible value {base:?} for hand with 2 jokers")
                    }
                }
            }
            3 => {
                // 1 three is caused by jokers
                match base {
                    HandStrength::T => HandStrength::F4,
                    HandStrength::FH => HandStrength::F5,
                    _ => {
                        panic!("impossible value {base:?} for hand with 3 jokers")
                    }
                }
            }
            4 | 5 => HandStrength::F5,
            _ => {
                panic!("impossible value {base:?} of jokers")
            }
        }
    }
    fn from_cards(cards: &[Face; 5]) -> Self {
        let values: Vec<u8> = cards
            .into_iter()
            .fold(HashMap::<Face, u8>::new(), |mut hm, &face| {
